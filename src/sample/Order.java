...
public class Order {
    ...
    public Order(String month_year , String date , String time) {
        ...
        ConfermationStage confermationStage = new ConfermationStage("Are You Sure You Want To Delete This Order?", "هل انت متاكد انك تريد مسح هذا طلب؟", 
                e->clearOrder());
        ...
        delete.setOnAction(e-> confermationStage.show());
    }
    ...
    public void setTextColor(){
        ...
        if(Main.doneLoadingOrders){
            if(Controller.iosToggleSwitch.isSwitchedOn())
                for(Object text : Controller.dataLayout.getChildren())                
                    ((Text)text).setStyle("-fx-fill:white");

            else
                for(Object text : Controller.dataLayout.getChildren())
                    ((Text)text).setStyle("-fx-text-fill:black");
        }
    ...
    public void deleteOrderNode(String name){
    ...
    public ObservableList<OrderNode> getOrderNodeList(){
        return list;
    }
    ...
    public class OrderNode {
        ...
        private SimpleStringProperty date = new SimpleStringProperty(Order.this.getDate() + " " + Order.this.getTime());
        ...
        public OrderNode(String name, double price, Label count) {
            ...
            count.textProperty().addListener(e -> {
                if (!isFinished() && !list.isEmpty()) {
                    amount.set(count.getText());
                    total.set(price * Double.parseDouble(amount.get()));
